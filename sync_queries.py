#!/usr/bin/env python3
"""Synchronise la table `queries` de la base du bot avec config/queries.yaml.

Remplace entièrement le contenu de la table à chaque exécution (idempotent) :
relancer ce script plusieurs fois avec le même YAML ne crée pas de doublons.
"""
...
def build_query_url(categorie, recherche, platform=None):
    params = {
        "catalog[]": categorie,
        "search_text": recherche["nom"],
    ...
    for section_name, section in config.items():
        ...
        for recherche in section.get("recherches", []):
            queries.append(
                {
                    "name": recherche["nom"],
                    "url": build_query_url(categorie, recherche, platform),
                }
            )
    return queries


def sync(queries, db_path, dry_run=False):
    if dry_run:
...
def main():
    ...
    sync(queries, args.db, dry_run=args.dry_run)
